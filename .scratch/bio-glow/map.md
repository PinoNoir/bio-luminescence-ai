...
- [Real species photos on detail and home pages](./issues/17-real-species-photos.md) — root-caused `speciesImages.ts`'s pipeline being almost entirely broken (EOL's classic API returns zero images for any of the 8 mock species — a dead API, not a CORS issue; GBIF has media for only 1/8; Unsplash/Pexels were never configured; the "fallback" was a generic unrelated wave photo shown for everything). Replaced with Wikipedia's REST API — correctly-matched photos for 6/8, no key needed. Wired into the species detail hero photo and the home page's featured-specimen block, both falling back to the honest glow-dot placeholder rather than ever showing a wrong photo.
