// PROTOTYPE — wipe me. Floating variant switcher for ticket 13. Hidden in production builds.
...
function PrototypeSwitcher({ variants, current }: PrototypeSwitcherProps) {
  const navigate = useNavigate({ from: '/prototype-sighting-form' });
  const search = useSearch({ from: '/prototype-sighting-form' });
  ...
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[999] flex items-center gap-3 rounded-full border border-yellow-400/40 bg-black shadow-[0_0_0_1px_rgba(250,204,21,0.15),0_8px_30px_rgba(0,0,0,0.6)] px-2 py-2">
      <button onClick={() => go(-1)} className="p-2 rounded-full text-yellow-300 hover:bg-yellow-400/10 transition-colors" aria-label="Previous variant">
      ...
      <button onClick={() => go(1)} className="p-2 rounded-full text-yellow-300 hover:bg-yellow-400/10 transition-colors" aria-label="Next variant">
