...
**Follow-up, same PR**: added a small circular photo thumbnail to each row (`SpeciesThumbnail`, using `useSpeciesImages`), falling back to the existing colored-dot treatment when no photo is found. This required fixing the underlying `speciesImages.ts` service first — see ticket 17, which root-caused why images were barely showing up anywhere in the app.
